...
class Documents extends Admin_Controller
{
    ...
    {
        $this->data['title'] = translate('Documents') . " " . translate('details');
        $this->data['doc_list'] = $this->documents_model->get_doucments_list();
        $this->data['sub_page'] = 'documents/view';
        $this->data['main_menu'] = 'documents';
    ...
    {
        $this->db->update('documents', ['is_read'=>1]);
        $this->db->update('chemical', ['unread'=>1]);
        $this->db->update('chemical_stock', ['unread'=>1]);
    ...
    public function create()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if (true) {
                ...
                $user_id = $this->documents_model->save_document($data);
                set_alert('success', translate('information_has_been_saved_successfully'));

                redirect(base_url('documents/view'));
            }
        }
        $this->data['stafflist'] = $this->employee_model->get_employee_list(0);
       
        $this->data['title'] = translate('Documents') . " " . translate('create');
        $this->data['sub_page'] = 'documents/create';
        $this->data['main_menu'] = 'documents';
        $this->load->view('layout/index', $this->data);
    }
        ...
        if (isset($_POST['update'])) {
          ...
          // $this->form_validation->run() !== false
            if (true) {
                ...
                redirect(base_url('documents/edit/' . $data['id']));
            } else {
                $this->session->set_flashdata('profile_tab', 1);
            }
        }
        $this->data['stafflist'] = $this->employee_model->get_employee_list(0);
        $this->data['doc_data'] = $this->documents_model->get_single_document($id);

        $this->data['title'] = translate('Documents') . " " . translate('Update');
        $this->data['sub_page'] = 'documents/edit';
        $this->data['main_menu'] = 'documents';
