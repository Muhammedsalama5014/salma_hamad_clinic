...
class Ajax extends Admin_Controller
{
    ...
    public function get_labtest_by_category()
    {
        ...
        $productlist = $this->db->select('id,name,test_code,from_date,to_date')->where('category_id', $category_id)->get('lab_test')->result_array();
        $html = "<option value=''>" . translate('select') . "</option>";
        foreach ($productlist as $product) {
            $selected = ($product['id'] == $selected_id ? 'selected' : '');
            $html .= "<option value='" . $product['id'] . "' " . $selected . " data-from_date='".$product['from_date']."' data-to_date='".$product['to_date']."'>" . $product['name'] . " (" . $product['test_code'] . ")</option>";
        }
        echo $html;
    }
