...
class Billing_model extends MY_Model
{
    ...
    public function save_test_bill($data)
    {
        ...
        if ($payment_amount > 0 && $payment_amount < $net_amount) {
            $status = 2;
        }
        ...
        $array_bill = array(
            ...
            'paid' => $payment_amount,
            'authorization_no' => $data['authorization_no'],
        ...
        foreach ($items as $key => $value) {
            ...
            $assignedResult = $this->db->get_where('chemical', array('category_id' => $value['category']))->result_array();
           
            foreach ($assignedResult as $assign) {
                $this->db->where('id', $assign['id']);
                $this->db->set('unit_ratio', 'unit_ratio - 1', false);
                $this->db->update('chemical');
                
                $chemical_stocks = $this->db->get_where('chemical_stock', array('chemical_id' => $assign['id']))->result_array();
               
                foreach ($chemical_stocks  as $chemicalstock ) {
                   
                    $this->db->where('id', $chemicalstock['id']);
                    $this->db->set('stock_quantity', 'stock_quantity - 1', false);
                    $this->db->update('chemical_stock');
                }
    ...
    public function save_billpayment($data)
    {
        ...
        if ($data['getbill']['due'] == $payment_amount || $data['getbill']['due'] == 0) {
            $status = 3;
        } else {
            $status = 2;
        }
    ...
    {
        $this->db->select('labtest_bill.*,SUM(total-discount+tax_amount) as net_amount,patient.name as p_name,patient.patient_id,patient.mobileno as p_mobileno,patient.nationalid as p_civil_id,patient.address as p_address,IFNULL(patient.age, "0") as p_age,patient.sex as p_sex,patient_category.name as p_category,staff.staff_id,staff.name as referral_name');
    ...
    {
        $this->db->select('labtest_bill.*,SUM(total-discount+tax_amount) as net_amount,patient.name as patient_name,patient.mobileno as patient_mobile,patient.nationalid as patient_civilid,patient.patient_id,staff.name as referral_name,labtest_report.delivery_date,labtest_report.delivery_time,labtest_report.status as delivery_status');
        $this->db->from('labtest_bill');
    ...
    public function get_due_collect_report($start = '', $end = '')
    {
		...
		labtest_bill.paid,labtest_bill.due,patient.name as patient_name,patient.mobileno as patient_mobile,patient.nationalid as patient_civilid,patient.patient_id, staff.name as collect_by,payment_type.name as pay_via FROM labtest_payment_history
    ...
    {
        $this->db->select('labtest_bill.*,SUM(total-discount+tax_amount) as net_amount,patient.name as patient_name,patient.mobileno as patient_mobile,patient.nationalid as patient_civilid,patient.patient_id,staff.name as referral_name');
        $this->db->from('labtest_bill');
        $this->db->join('patient', 'patient.id = labtest_bill.patient_id', 'left');
        $this->db->join('staff', 'staff.id = labtest_bill.referral_id', 'left');
        //$this->db->join('labtest_bill_details', 'labtest_bill.id = labtest_bill_details.labtest_bill_id', 'inner');
        if ($mode == 'due') {
    ...
    public function get_category_bill_list($bill_id = '')
    {
        $this->db->select('labtest_bill_details.*,lab_test.name as test_name,lab_test_category.name as test_category');
        $this->db->from('labtest_bill_details');
        $this->db->join('lab_test', 'lab_test.id = labtest_bill_details.test_id', 'left');
        $this->db->join('lab_test_category', 'lab_test_category.id = labtest_bill_details.category_id', 'left');
        $this->db->where('labtest_bill_details.labtest_bill_id', $bill_id);
        $this->db->order_by('labtest_bill_details.id', 'DESC');
        $results = $this->db->get()->row_array();
        return  $results['test_category'];
    }
}
