...
class Inventory_model extends MY_Model
{
    ...
    public function save_chemical($data)
    {
        $insert_chemical = array(
            ...
            'unit_ratio2' => $data['unit_ratio'],            
            'purchase_price' => $data['purchase_price'],
            'sales_price' => $data['sales_price'],
            'remarks' => $data['remarks'],
            'unread' => 0,
            'admin_not_datetime' => date('Y-m-d H:i', strtotime($data['admin_not_datetime'])),
            'min_qty_noti' => $data['min_qty_noti'],
        );
        if (isset($data['chemical_id']) && !empty($data['chemical_id'])) {
            ...
            $insert_data = array(
                ...
                'admin_not_datetime' => $data['admin_not_datetime'],
                'min_qty_noti' => $data['min_qty_noti'],
            );
    ...
    public function save_purchase($data)
    {
        ...
        foreach ($purchases as $key => $value) {
            ...
            $this->stock_upgrade($value['quantity'], $value['chemical']);
        }
    ...
    {
        $this->db->select('chemical_stock.*,chemical.name as chemical_name,chemical.available_stock as available_stock,chemical_category.name as category_name,staff.name as staff_name,staff.staff_id');
    ...
    public function save_stock($data)
    {
        ...
        if (isset($data['old_chemical_id'])) {
            if ($data['old_chemical_id'] == $data['chemical_id']) {
                ...
                $this->stock_upgrade_update($data['stock_quantity'], $data['chemical_id']);
            } else {
    ...
    public function stock_upgrade_update($stock, $chemical_id)
    {
        $this->db->set('available_stock', $stock, false);
        $this->db->where('id', $chemical_id);
        $this->db->update('chemical');
    }
