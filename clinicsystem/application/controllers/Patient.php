...
class Patient extends Admin_Controller
{
    ...
    public function checkbyphone() 
    {
        ...
        $patientlist = $this->patient_model->get_patient_list_by_number($_POST['phone']);
    ...
    public function search()
    {
     ...
     public function callback_mobile_no () {
        
        $this->db->select('*');
        $this->db->from('patient');
        $this->db->where('mobileno', $_POST['mobile_no']);
    ...
    public function callback_code_no () {
    ...
    public function callback_civil_id () {
    
    $this->db->select('*');
    $this->db->from('patient');
    $this->db->where('nationalid', $_POST['nationalid']);
    $data = $this->db->get()->result_array();
    ...
    public function create()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            $this->form_validation->set_rules('retype_password', 'Retype Password');
            //, 'trim|required|matches[password]'
            //$validPhone = $this->callback_mobile_no();

            $data = $this->input->post();
           
            /*if ($validPhone==='no') {
                set_alert('error', translate('Patient already exist with this phone number'));
                $this->session->set_flashdata('data',$data);
                redirect(base_url('patient/create/'));
            } */           
            
            if($data['patient_id'] != null){
                $validcode = $this->callback_code_no();
                if ($validcode ==='no') {
                    set_alert('error', translate('Patient already exist with this Patient file number')); 
                    $this->session->set_flashdata('data',$data);
                    redirect(base_url('patient/create/'));
                }    
            }
            ...
            if($data['nationalid'] != null){
                $validcode = $this->callback_civil_id();
                if ($validcode ==='no') {
                    set_alert('error', translate('Patient already exist with this Civil id number')); 
                    $this->session->set_flashdata('data',$data);
                    redirect(base_url('patient/create/'));
                }    
            }
    ...
    public function profile($id = '')
    {
        ...
        $this->data['patient'] = $this->patient_model->get_single_patient2($id);
        $this->data['billlist'] = $this->patient_model->get_bill_list($id);
        $this->data['categorylist'] = $this->app_lib->getSelectList('patient_category');

        $this->data['appointmentlist'] = $this->appointment_model->get_appointment_list(true, '', '', $id);
