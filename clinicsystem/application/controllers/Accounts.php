...
class Accounts extends Admin_Controller
{
    ...
    public function refund_amount ( $id ) {
        ...
        $data = $this->accounts_model->get_voucher_by_id($id);
        if (isset($data[0])) {
            $post_data = [
                'account_id'=>$data[0]['account_id'],
                'voucher_head_id'=>$data[0]['voucher_head_id'],
                'voucher_type'=>'expense',
                'ref_no'=>$data[0]['ref'],
                'amount'=>$data[0]['amount'],
                'pay_via'=>$data[0]['pay_via'],
                'is_refund'=>'1',
                'description'=>'Refund Amount For Ref # '.$data[0]['ref'],
                'date'=>date('Y-m-d'),
                
            ];
            $this->accounts_model->save_voucher($post_data);
            $this->db->where('id', $id);
            $this->db->update('transactions', array('is_refund' => '1'));

            set_alert('success', "Payment has been refunded");
            redirect('accounts/voucher');
        }
