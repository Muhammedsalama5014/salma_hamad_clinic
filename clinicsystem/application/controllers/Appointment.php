...
class Appointment extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $login_user_id = $this->session->userdata('loggedin_id');
        $is_superadmin_edit = '0';
        if (loggedin_role_id()==1 || loggedin_role_id()==2)
        {
            $is_superadmin_edit = '1';
        }
        
        $this->data['login_user_id'] = $login_user_id;
        $this->data['is_superadmin_edit'] = $is_superadmin_edit;
    ...
    public function add()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            $created_by_user_id = $this->session->userdata('loggedin_id');
        ...
        $this->data['services'] = $this->test_model->getLabTestBetween();
    ...
    public function edit($id = '')
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if($data['status'] == 7){
                $appointment_details = $this->appointment_model->get_my_appointment_details($id);
                $patient_id = $appointment_details->patient_id;    
                $add_general_patient_id = $this->patient_model->addGeneralPatientIdNew($patient_id);
            }
    ...
    public function schedule_closed($id = '')
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('appointment', array('status' => 7));
