...
function discount_update(rowid) {
    var unit_quantity = read_number($('#unit_quantity' + rowid).val());
    var unit_price = read_number($('#unit_price' + rowid).val());
    var discount_percent = read_number($('#dis_percent' + rowid).val());
    var dis_amount0 = read_number($('#dis_amount' + rowid).val());
    var total_price_count = unit_quantity * unit_price;
    if (dis_amount0 > total_price_count) {
        alert("Discount must be less then unit price");
        $('#dis_amount' + rowid).val("0.00")
        return;
    }

    var discount_percent = (dis_amount0 / total_price_count) * 100;
    $('#dis_percent' + rowid).val(discount_percent.toFixed(2));
    $('#total_price' + rowid).val(total_price_count - dis_amount0);
    $('#hidden_total_price' + rowid).val(total_price_count);
...
// test bill discount calculation
function age_update() {
    var patient_birthday = read_number($('#patient_birthday').val());
    var age = read_number($('#age').val());
    var nationalid = read_number($('#nationalid').val());
    ...
    var patient_year = nationalid.slice(1, 3);
    var patient_month = nationalid.slice(3, 5);
    var patient_day = nationalid.slice(5, 7);
    var current_patient_birthday = patient_year + '/' + patient_month + '/' + patient_day;

    if (current_patient_birthday < today) {
        alert("Patient birthday must be less then today date");
        return;
    }

    var old_patient_birthday = new Date(current_patient_birthday);
    old_patient_birthday.setDate(old_patient_birthday.getDate() + 1);
    patient_birthday = new Date(old_patient_birthday).toISOString().slice(0, 10);

    var patient_birthday_2 = new Date(patient_birthday);
    var today_2 = new Date();
    age = parseInt((today_2 - patient_birthday_2) / (1000 * 60 * 60 * 24), 10);
    age = Math.abs(Math.round(age / 365.25));

    $('#age').val(age);
    $('#patient_birthday').val(patient_birthday);
}
